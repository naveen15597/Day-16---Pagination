...
// const users1= fetch('https://616a9d5616e7120017fa1032.mockapi.io/user').then((res)=> res.json()).then((res) => {
//     users= res;
...
// });
const num = document.getElementById('numbers');
num.innerHTML=`<button class="btn btn-outline-primary" onclick="changePage('prev')">Prev</button> `
for(let i=1;i<=users.length/10;i++)
{
    num.innerHTML += `    
<button id=${i} class="btn btn-outline-success" onclick="changePage(event)">${i}</button>
`
}

num.innerHTML+=`<button class="btn btn-outline-primary" onclick="changePage('next')">Next</button> `
...
function changePage(direction){
    ...
    if(direction=="next"){
        currPageNo = currPageNo < users.length/pageSize -1 ? currPageNo+ 1: currPageNo;
        if(currPageNo*pageSize+10<= users.length){
            print();
        }
        ...
        currPageNo =document.getElementById('search').value;
        print();
    }
    else{        
        let ctr=0;
        let value = direction.target.textContent;
        currPageNo = Number(value)-1;
        // const stl = document.getElementById(`${currPageNo+ 1}`);
        // stl.style.background = 'green';        
        // var link = document.getElementsByClassName("btn btn-primary");
    //     for (var i = 0; i < link.length; i++) {
    //         if(link[i].classList.contains('btn-back-color')){
    //             link[i].classList.remove('btn-back-color');  
    //             stl.classList.add('btn-back-color');
    //             ctr=1;                
    //         }
    //         // if(link[i].classList.contains("active"))
    //         // {
    //         //     link[i].classList.remove('active');  
    //         //     link[evt- 1].classList.add('active');          
    //         //     break;
    //         // }       
    //   }    
    //   if(ctr==0){
...
function print(){
    tbody.innerHTML = "";
    for(let i=currPageNo*pageSize; i<currPageNo*pageSize+10;i++){
        insertRow(users[i]);
    }
}